...
import { getOutOfStockProducts, getProductById } from "../entities/dbProducts";
...
export const generateReportOutOfStockProducts = async () => {
    ...
    //Header
    doc.setFontSize(12)
    xPosText = centerText('Productos agotados', doc)
    doc.text(xPosText, 30, 'Productos agotados')
    //Products
    let productsList = []
    for(let i=0; i<list.length; i++){
        productsList.push([(i+1), list[i].name, list[i].brand, list[i].salesUnit, list[i].supplier])
    }
    doc.autoTable({
        head: [['N°', 'Nombre', 'Marca', 'Presentación', 'Proveedor']],
        body: productsList,
        margin: {
            top: 35,
            left: 20,
...
async function gettingOutOfStockProducts(){
    const snapshot =  await getOutOfStockProducts()
    let array = []
